...
const Ostelin60VTab: React.FC<Props> = ({
  ...
  const [programTabInternal, setProgramTabInternal] = useState<GoiHangProgramTab>('ostelin');
  ...
  const filteredPharmatonRows = useMemo(
    () => allPharmatonRows.filter(r => repNameMatchesEmployee(r.rep, currentEmployee)),
    [allPharmatonRows, currentEmployee]
  );

  /** THỐNG KÊ THEO REP — PMT Vỉ (toàn bộ Rep) */
      ...
      <div className="flex flex-col gap-4">
            ...
            <h2 className="text-lg font-black uppercase flex items-center gap-2">
              ...
              <span className={isOstelinTab ? 'text-opella-green dark:text-opella-green' : 'text-violet-800 dark:text-violet-200'}>
                {isOstelinTab
                  ...
                  : 'THEO DÕI GÓI PHARMATON VỈ (5H)'}
              </span>
            </h2>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
              {isOstelinTab ? (
                <>
                  Đợt 2 từ {dot2StartLabel} — sheet <span className="font-mono">{OSTELIN_60V_GOI_SHEET}</span>
                  , cột <span className="font-mono">Dot_2</span>
                </>
              ) : (
                <>
                  Sheet <span className="font-mono">{PHARMATON_VI_GOI_SHEET}</span> · Mua ≥ 5 hộp PHARMATON VITALITY BLISTER
                </>
              )}
                ...
                <table className="w-full text-left text-sm border-collapse min-w-[560px]">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                    {pharmatonByRep.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="px-4 py-8 text-center text-slate-400 italic">Chưa có dữ liệu gói PMT Vỉ</td>
                      </tr>
        ...
        {!loading && !error && !isOstelinTab && (
            <div className="border border-violet-200 dark:border-violet-800 rounded-xl overflow-hidden">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-violet-50/60 dark:bg-violet-950/25">
                <div>
                ...
                <table className="w-full text-left text-sm border-collapse min-w-[720px]">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                    {filteredPharmatonRows.length === 0 ? (
                      <tr>
                        <td colSpan={8} className="px-4 py-8 text-center text-slate-400 italic">Chưa có KH mua gói PMT Vỉ</td>
                      </tr>
